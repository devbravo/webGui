...
// Perform the check, send the orginalCheckData and the outcome of the check process
workers.performCheck = function (originalCheckData) {
  ...
  // Mark that the outcome hasn't been send yet.
  ...
  // Instantiate the request object (using either the http or https modoule)
  ...
  // Bind to the error even so it doens't get thrown
...
workers.processCheckOutcome = async (originalCheckData, checkOutcome) => {
  ...
  // Save the updates
  const updatedData = await _data.update('checks', newCheckData.id, newCheckData);
...
workers.log = async (originalCheckData, checkOutcome, state, alertWarranted, timeOfCheck) => {
  ...
  const appendFile = await _logs.append(logFileName, logString);
...
workers.rotateLogs = async () => {
  ...
  if (logs && logs.length > 0) {
    logs.forEach(async logName => {
      ...
      const compressLog = await _logs.compress(logId, newFileId);

      if (typeof compressLog == 'undefined') {
        // Truncating the log
        const truncateLog = await _logs.truncate(logId);
        if (typeof truncateLog == 'undefined') {
